...
use std::path::Path;
use validation::{is_potential_image_file, validate_input_path, validate_output_path};
...
fn main() -> Result<()> {
    ...
    match args.command {
        Commands::Compress {
            ...
            let options =
                CompressionOptions::new(quality, width, height, format, exact_resize, dry_run)?;
