...
use img_squeeze::processing::{CompressionOptions, load_image_with_metadata, resize_image, process_and_save_image};
...
fn bench_image_loading(c: &mut Criterion) {
    ...
    c.bench_function("image_loading", |b| {
        b.iter(|| {
            load_image_with_metadata(black_box(&test_file))
        })
    });
}
...
fn bench_image_resizing(c: &mut Criterion) {
    ...
    for size in [Small, Medium, Large].iter() {
        ...
        if let Ok((img, _)) = load_image_with_metadata(&test_file) {
            let options = CompressionOptions::new(Some(80), Some(width / 2), Some(height / 2), None).unwrap();
            
            group.bench_with_input(BenchmarkId::new("resize", format!("{}x{}", width, height)), &(img, options), |b, (img, options)| {
                b.iter(|| {
...
fn bench_batch_processing(c: &mut Criterion) {
    ...
    c.bench_function("batch_processing", |b| {
        b.iter(|| {
            ...
            let files: Vec<_> = std::fs::read_dir(temp_dir.path()).unwrap()
